...
# Reason for less datapoints is due to a larger number og demographic data than in the other datasets. Therefore, it cannot be merged. 
```
...
CleanData %>% group_by(ID) %>% summarise(MEAN=mean(CHI_MLU)) %>% filter(MEAN > 2.7)
...
#ID 55 produced most words (1294)
...
#Adding a column in the data set containing the mean amount of words produced by each trial across all visits
